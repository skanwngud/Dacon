...
labels_df = pd.read_csv('c:/data/dacon/data2/dirty_mnist_answer.csv')[:]
imgs_dir = np.array(sorted(glob.glob('c:/data/dacon/data2/dirty_mnist/*')))[:]
labels = np.array(labels_df.values[:,1:])

test_imgs_dir = np.array(sorted(glob.glob('c:/data/dacon/data2/test_dirty_mnist/*')))
...
#     model = nn.DataParallel(model)
    ...
    for epoch in range(epochs):
        ...
        if np.mean(valid_batch_accuracy)>valid_best_accuracy:
            torch.save(model.state_dict(), 'C:/Users/Yoo/.cache/torch/hub/checkpoints/EfficientNetB0-fold{}.pt'.format(fold))
...
submission = pd.read_csv('c:/data/dacon/data2/sample_submission.csv')

with torch.no_grad():
    for fold in range(1):
        model = EfficientNet_MultiLabel(in_channels=3).to(device)
        model.load_state_dict(torch.load('C:/Users/Yoo/.cache/torch/hub/checkpoints/EfficientNetB0-fold{}.pt'.format(fold)))
        model.eval()
...
submission.to_csv('c:/data/dacon/data2/EfficientNetB0-fold5.csv', index=False)