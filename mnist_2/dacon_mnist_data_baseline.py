...
dirty_mnist_answer = pd.read_csv("c:/data/dacon/data2/dirty_mnist_answer.csv")
# dirty_mnist라는 디렉터리 속에 들어있는 파일들의 이름을 
# namelist라는 변수에 저장
namelist = os.listdir('c:/data/dacon/data2/dirty_mnist/')
...
#test Dataset 정의
sample_submission = pd.read_csv("c:/data/dacon/data2/sample_submission.csv")
test_dataset = DatasetMNIST("c:/data/dacon/data2/test_dirty_mnist/", sample_submission)
batch_size = 128
...
predictions_list = []
# 배치 단위로 추론
prediction_df = pd.read_csv("c:/data/dacon/data2/sample_submission.csv")
...
sample_submission.to_csv("c:/data/dacon/data2/baseline_prediction.csv", index = False)
sample_submission